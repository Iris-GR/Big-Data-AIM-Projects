...
ratio <- nr_class_y[2]/(nr_class_y[1]+nr_class_y[2])
ratio

cor_pred <- cor(X)


highCorr <- sum(abs(cor_pred[upper.tri(cor_pred)]) > .9)




corrplot(cor_pred)
sum_pred_cor <- summary(cor_pred[upper.tri(cor_pred)])
sum_pred_cor


highlyCor <- findCorrelation(cor_pred, cutoff = .75)

fil <- X[,-highlyCor]

cor_pred_fil <- cor(fil)
corrplot(cor_pred_fil, method = 'number')
